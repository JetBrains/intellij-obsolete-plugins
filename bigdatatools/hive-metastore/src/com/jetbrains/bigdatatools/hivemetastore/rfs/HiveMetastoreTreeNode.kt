...
import com.jetbrains.bigdatatools.hivemetastore.utils.HiveMetastoreUtils.isSchema
import com.jetbrains.bigdatatools.hivemetastore.utils.HiveMetastoreUtils.isTable

class HiveMetastoreTreeNode(project: Project,
                            rfsPath: RfsPath,
                            driver: HiveMetastoreDriver) : DriverFileRfsTreeNode(project, rfsPath, driver) {
  private val focusId = driver.connectionData.innerId

  override fun onDoubleClick(): Boolean {
    if (rfsPath.isRoot) {
      val project = project ?: return true
      val controller = (driver as HiveMetastoreDriver).getController(project)
  ...
  override fun getIdleIcon() = when {
    ...
    rfsPath.isCatalog -> BigdatatoolsCoreIcons.Nodes.Catalogs
