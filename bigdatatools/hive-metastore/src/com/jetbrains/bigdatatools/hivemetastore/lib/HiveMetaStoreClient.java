...
// This file has been modified by JetBrains.

package com.jetbrains.bigdatatools.hivemetastore.lib;
...
/**
 * Clone of Hive Metastore Client to support Hive V2
 */
...
public class HiveMetaStoreClient implements IMetaStoreClient, AutoCloseable {
   ...
   */
  public static final ClientCapabilities VERSION = new ClientCapabilities(
    List.of(ClientCapability.INSERT_ONLY_TABLES));
  // Test capability for tests.
  public static final ClientCapabilities TEST_VERSION = new ClientCapabilities(
    List.of(ClientCapability.INSERT_ONLY_TABLES, ClientCapability.TEST_CAPABILITY));
  protected static final Logger LOG = Logger.getInstance(HiveMetaStoreClient.class);
  ...
  public boolean isV3Connection = true;
  ...
  public HiveMetaStoreClient(Configuration conf, HiveMetaHookLoader hookLoader, Boolean allowEmbedded)
    ...
    if (localMetaStore) {
      ...
      Method createMethod;
      try {
        //noinspection JavaReflectionMemberAccess,VulnerableCodeUsages
        createMethod = HiveMetaStore.class.getMethod("newRetryingHMSHandler", Configuration.class, Boolean.class);
      ...
      try {
        client = (ThriftHiveMetastore.Iface)createMethod.invoke("hive client", this.conf, true);
      }
  ...
  private synchronized URIResolverHook loadUriResolverHook() throws IllegalStateException {
    ...
    if (uriResolverClassName.isEmpty()) {
      return null;
    }
    else {
  ...
  public boolean isCompatibleWith(Configuration conf) {
    ...
    for (ConfVars oneVar : MetastoreConf.metaVars) {
      ...
      if (oldVar == null ||
        ...
        LOG.info("Mestastore configuration " + oneVar +
  ...
  public void alter_table(String catName, String dbName, String tblName, Table newTable,
    ...
    client.alter_table_with_environment_context(prependCatalogToDbName(catName, dbName), tblName, newTable, envContext);
  }
  ...
  private void open() throws MetaException {
          ...
          if (useSSL) {
            try {
              ...
              String trustStorePassword =
                MetastoreConf.getPassword(conf, ConfVars.SSL_TRUSTSTORE_PASSWORD);
    ...
    if (!isConnected) {
      String error = tte != null ? StringUtils.stringifyException(tte) : "";
      throw new MetaException("Could not connect to meta store using any of the URIs provided." +
                              " Most recent failure: " + error);
    }
  ...
  public Catalog getCatalog(String catName) throws TException {
    if (!isV3Connection) {
      return new Catalog(Warehouse.DEFAULT_CATALOG_NAME, "");
    }
  ...
  public List<String> getCatalogs() throws TException {
    if (!isV3Connection) {
      return List.of(Warehouse.DEFAULT_CATALOG_NAME);
    }
  ...
  public Partition appendPartition(String catName, String dbName, String tableName,
    ...
    Partition p = client.append_partition_by_name(prependCatalogToDbName(catName, dbName), tableName, name);
    return deepCopy(p);
  }
  ...
  public Partition appendPartition(String catName, String dbName, String tableName,
    ...
    Partition p = client.append_partition(prependCatalogToDbName(catName, dbName), tableName, partVals);
    return deepCopy(p);
  }
  ...
  public boolean dropPartition(String catName, String db_name, String tbl_name, String name,
    ...
    return client.drop_partition_by_name_with_environment_context(prependCatalogToDbName(catName, db_name), tbl_name, name, deleteData,
                                                                  null);
  }
  ...
  public boolean dropPartition(String catName, String db_name, String tbl_name,
    ...
    return client.drop_partition_with_environment_context(prependCatalogToDbName(catName, db_name), tbl_name, part_vals, options.deleteData,
  ...
  public List<String> getDatabases(String catName, String databasePattern) throws TException {
    return filterHook.filterDatabases(client.get_databases(prependCatalogToDbName(catName, databasePattern)));
  }

  private @NotNull String prependCatalogToDbName(String catName, String databasePattern) {
    if (isV3Connection) {
      return MetaStoreUtils.prependCatalogToDbName(catName, databasePattern, conf);
    }
    else {
  ...
  private String prependCatalogToDbName(String catalogName, String dbName,
                                        Configuration conf) {
    if (isV3Connection) {
      return MetaStoreUtils.prependCatalogToDbName(catalogName, dbName, conf);
    }
    else {
      return dbName;
    }
  ...
  public String prependCatalogToDbName(String dbName, Configuration conf) {
  ...
  public List<Partition> listPartitionsWithAuthInfo(String catName, String dbName, String tableName,
    ...
    List<Partition> parts = client.get_partitions_with_auth(prependCatalogToDbName(catName, dbName), tableName, shrinkMaxtoShort(maxParts),
                                                            userName, groupNames);
    return deepCopyPartitions(filterHook.filterPartitions(parts));
  }
  ...
  public List<Partition> listPartitionsWithAuthInfo(String catName, String dbName, String tableName,
    ...
    throws TException {
    List<Partition> parts = client.get_partitions_ps_with_auth(prependCatalogToDbName(catName, dbName), tableName, partialPvals,
                                                               shrinkMaxtoShort(maxParts), userName, groupNames);
    return deepCopyPartitions(filterHook.filterPartitions(parts));
  }
  ...
  public List<Partition> listPartitionsByFilter(String catName, String db_name, String tbl_name,
    ...
    List<Partition> parts =
      client.get_partitions_by_filter(prependCatalogToDbName(catName, db_name), tbl_name, filter, shrinkMaxtoShort(max_parts));
    return deepCopyPartitions(filterHook.filterPartitions(parts));
  }
  ...
  public List<TableMeta> getTableMeta(String catName, String dbPatterns, String tablePatterns,
    ...
    return filterHook.filterTableMetas(client.get_table_meta(prependCatalogToDbName(catName, dbPatterns), tablePatterns, tableTypes));
  }
  ...
  public List<String> listPartitionNames(String catName, String dbName, String tableName,
    ...
    return filterHook.filterPartitionNames(catName, dbName, tableName,
                                           client.get_partition_names(prependCatalogToDbName(catName, dbName), tableName,
  ...
  public List<FieldSchema> getSchema(String catName, String db, String tableName) throws TException {
    ...
    if (addedJars != null && !addedJars.isBlank()) {
      Map<String, String> props = new HashMap<>();
      props.put("hive.added.jars.path", addedJars);
      envCxt = new EnvironmentContext(props);
    }

    List<FieldSchema> fields = client.get_schema_with_environment_context(prependCatalogToDbName(catName, db), tableName, envCxt);
    return deepCopyFieldSchemas(fields);
  }
  ...
  private static Database deepCopy(Database database) {
    Database copy = null;
    if (database != null) {
  ...
  private static FieldSchema deepCopy(FieldSchema schema) {
  ...
  private static Function deepCopy(Function func) {
    Function copy = null;
    if (func != null) {
  ...
  protected static List<FieldSchema> deepCopyFieldSchemas(List<FieldSchema> schemas) {
  ...
  public boolean refresh_privileges(HiveObjectRef objToRefresh, String authorizer,
                                    PrivilegeBag grantPrivileges) throws
                                                                  TException {
    String defaultCat = getDefaultCatalog(conf);
  ...
  public String[] getMasterKeys() throws TException {
    ...
    return ArrayUtil.toStringArray(keyList);
  }
  ...
  public List<Long> replOpenTxn(String replPolicy, List<Long> srcTxnIds, String user) throws TException {
    // As this is called from a replication task, the user is the user who has fired the repl command.
  ...
  @Override
  public void replRollbackTxn(long srcTxnId, String replPolicy) throws TException {
  ...
  public void insertTable(Table table, boolean overwrite) throws MetaException {
    ...
    if (!(hook instanceof DefaultHiveMetaHook hiveMetaHook)) return;
    try {
      ...
      for (NotificationEvent e : rsp.getEvents()) {
        if (e.getEventId() != nextEventId) {
          LOG.error("Requested events are found missing in NOTIFICATION_LOG table. Expected: {}, Actual: {}. "
                    ...
                    + "Also, bootstrap the system again to get back the consistent replicated state." + nextEventId + e.getEventId());
  ...
  @Override
  public WMFullResourcePlan getActiveResourcePlan() throws TException {
  ...
  private static short shrinkMaxtoShort(int max) {
    if (max < 0) {
      return -1;
    }
