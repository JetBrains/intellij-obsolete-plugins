...
data class DfTypeCheckAliasAlreadyExists(val place: String,
                                         val name: String,
  ...
  override fun logUsage(expr: ScExpression) {
    SparkStatisticScala.logInspection(expr, name, SparkDataFrameInspection.ALIAS_EXISTS)
  }

  override fun errorMessage(): String =
    SparkScalaMessagesBundle.message("df.type.check.error.alias.already.exists", name)

  override fun equals(other: Any?): Boolean {
    if (this === other) return true
    if (javaClass != other?.javaClass) return false

    other as DfTypeCheckAliasAlreadyExists
