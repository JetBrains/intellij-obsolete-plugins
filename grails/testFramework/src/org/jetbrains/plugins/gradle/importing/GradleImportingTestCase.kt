...
abstract class GradleImportingTestCase : JavaExternalSystemImportingTestCase() {
    ...
    private val versionMatcherRule: VersionMatcherRule =
        asOuterRule<VersionMatcherRule>(VersionMatcherRule())
    private val myTargetJavaVersionWatcher: TargetJavaVersionWatcher =
        asOuterRule<TargetJavaVersionWatcher>(
    ...
    override fun setUp() {
        ...
        ExtensionTestUtil.maskExtensions<UnknownSdkResolver>(
            UnknownSdkResolver.EP_NAME,
            listOf(TestUnknownSdkResolver),
            myTestDisposable
        )
        setRegistryPropertyForTest("unknown.sdk.auto", "false")
        TestUnknownSdkResolver.unknownSdkFixMode = TestUnknownSdkResolver.TestUnknownSdkFixMode.REAL_LOCAL_FIX
    ...
    override fun handleImportFailure(errorMessage: String, errorDetails: String?) {
        ...
        if (combinedMessage.contains("org.gradle.wrapper.Download.download") && combinedMessage.contains("java.net.SocketException")) {
            ExternalResourcesChecker.reportUnavailability<Any>("Gradle distribution service", null)
        }
    ...
    protected open fun installGradleJvmConfigurator() {
        ExternalSystemApiUtil.subscribe(myProject, GradleConstants.SYSTEM_ID, object :
            ExternalSystemSettingsListener<GradleProjectSettings> {
    ...
    private fun createJdkFromJavaHome(): Sdk {
        ...
        TestCase.assertNotNull("Cannot create JDK for " + this.gradleJdkHome, jdk)
        return jdk!!
    }
            ...
            if (eelJdkPath != null) {
                ...
                TestCase.assertTrue(
                    """
                      ...
                      """.trimIndent(),
                    isSupported(gradleVersion, eelJdkVersion) && !javaVersionRestriction.isRestricted(
                        gradleVersion,
                        eelJdkVersion
                    )
                )
