...
public class ArtifactsGenerator {
  ...
  private Target createArtifactTarget(Artifact artifact) {
    ...
    if (needAntArtifactInstructions(artifact.getArtifactType())) {
      copyInstructions.addAll(PackagingElementAntGenerators.computeAntInstructions(artifact.getRootElement(), myResolvingContext, creator, myContext, artifact.getArtifactType()));
    }
