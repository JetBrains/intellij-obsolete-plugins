...
public class GenerateAntBuildFileAction extends CompileActionBase {
  ...
  private static boolean validateGenOptions(Project project, GenerationOptionsImpl genOptions) {
    ...
    if (!conflicts.isEmpty()) {
      ...
      for (String conflictingChunk : conflicts) {
        msg.append(GenerateAntBundle.message("generate.ant.build.custom.compiler.conflict.message.row", conflictingChunk));
      }
      int rc = Messages
        .showOkCancelDialog(project, GenerateAntBundle.message("generate.ant.build.custom.compiler.conflict.message", msg.toString()),
                            GenerateAntBundle.message("generate.ant.build.custom.compiler.conflict.title"), Messages.getErrorIcon());
      if (rc != Messages.OK) {
        return false;
      }
    }
  ...
  private void generate(@NotNull Project project, final GenerationOptions genOptions) {
    ...
    try {
      ...
      new Task.Modal(project, GenerateAntBundle.message("generate.ant.build.title"), false) {
        @Override
        public void run(@NotNull final ProgressIndicator indicator) {
          indicator.setIndeterminate(true);
          indicator.setText(GenerateAntBundle.message("generate.ant.build.progress.message"));
          try {
    ...
    if (_ex[0] != null) {
      Messages.showErrorDialog(project, GenerateAntBundle.message("error.ant.files.generate.failed", _ex[0].getMessage()),
                               GenerateAntBundle.message("generate.ant.build.title"));
    }
    else {
      ...
      Messages.showInfoMessage(project, GenerateAntBundle.message("message.ant.files.generated.ok", filesString.toString()),
                               GenerateAntBundle.message("generate.ant.build.title"));
    }
  ...
  private boolean backup(final File file, final Project project, GenerationOptions genOptions, List<? super File> filesToRefresh) {
    ...
    try {
      ...
      Messages.showErrorDialog(project, GenerateAntBundle.message("error.ant.files.backup.failed", path),
                               GenerateAntBundle.message("generate.ant.build.title"));
      ok = false;
    }
